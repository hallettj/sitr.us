...
The hash here as a recursive hash of a directory of downloaded NPM packages that
...
It would be great if configure Dependabot to run a custom shell command along
with its updates.
...
That workflow includes `[dependabot skip]` in its commit messages to signal to
